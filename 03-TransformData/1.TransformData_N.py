...
spark.sql("select distinct Parameter_Name,Parameter_Name_fix from TAB").write.mode("overwrite").csv(ResearchPath+"/production_run/lz_parameters_names/distinct/")
...
sql = '''
  ...
  where upper(Parameter_Name) like "%_TEMP%") 
'''
...
Columns_less_the_two_unique_values = ['AIR_DR_IN_TEMP_Mean', 'AIR_DR_IN_TEMP_Median', 'AIR_DR_IN_VELOCITY_Mean', 'AIR_DR_OUT_TEMP_Mean', 'AIR_DR_OUT_TEMP_Median', 'AIR_DR_OUT_VELOCITY_Mean', 'AIR_UV_LMP_COOLING_IN_TEMP_Mean', 'AIR_UV_LMP_COOLING_IN_TEMP_Median', 'AIR_UV_LMP_COOLING_IN_VELOCITY_Mean', 'AIR_UV_LMP_COOLING_OUT_TEMP_Mean', 'AIR_UV_LMP_COOLING_OUT_TEMP_Median', 'AIR_UV_LMP_COOLING_OUT_VELOCITY_Mean', 'AIR_UV_QRZ_COOLING_TEMP_Mean', 'AIR_UV_QRZ_COOLING_TEMP_Median', 'AIR_UV_QRZ_COOLING_VELOCITY_Mean', 'AIR_UV_SOLV_OUT_TEMP_Mean', 'AIR_UV_SOLV_OUT_TEMP_Median', 'AIR_UV_SOLV_OUT_VELOCITY_Mean', 'CTR_2_GRVR_ROLL_TORQUE_SP_Mean', 'CTR_4_GRVR_SPEED_FDBK_Mean', 'DR_EXHAUST_SPEED_FDBK_Mean', 'DR_SUPPLY_SPEED_FB_Mean', 'DR_VELOCITY_FB_Mean', 'Laminator_IMP_Rel_Speed_%_Mean', 'Laminator_IMP_Torque_%_Mean', 'UV_LAMP_POWER_Mean', 'UV_LAMP_TEMP_Mean', 'UV_LAMP_TEMP_Median', 'UV_LAMP_VOLTS_Mean']
...
print(f'total columns droped:{len(Columns_less_the_two_unique_values)}\nlist of columns Droped by unique values: {Columns_less_the_two_unique_values}\ndata frame structure after droping  {parameters_run_level_pivot.count(),len(parameters_run_level_pivot.columns)}')
...
'''select Run_Number,cast((Sum(BLK_Quality_Status_Flag) / Count(Blanket_SEQ_NR)) AS FLOAT) AS Yield from blankets_db.lz_new_era_hist 
where BLK_Quality_Status_Flag in ("1","0")
Group by Run_Number''')
...
Blankets_Production_lifespan_df.coalesce(1).write.format("parquet") \
.option("path",certified_pit + "/Blankets_Production_lifespan/ce_fact_Blankets_Production_lifespan/") \
.mode("overwrite") \
.saveAsTable("blankets_db.ce_fact_Blankets_Production_lifespan")

# COMMAND ----------

# MAGIC %md 
# MAGIC ## create dataset with all the blankets and the parameters values
...
SELECT
  ...
  Blanket_Legacy_Part_Nr ,
  Product_Engineering_Name,
  BLK_Quality_Status_Flag
  from ld_New_Era_df_view
  where BLK_Quality_Status_Flag not in ("2","99999")
  AND Run_Number is not null
'''
ld_fact_New_Era_all_blankets_df = spark.sql(sql)
...
sql = '''
select * from (
select distinct Product_Engineering_Name,
CASE When Product_Engineering_Name LIKE '%Gem%' Then   'GEMINI3'
      when Product_Engineering_Name LIKE '%Timna%' Then 'TIMNA 2'
	  when Product_Engineering_Name LIKE '%Iris%' Then   'IRIS PLUS_SP'
	  when Product_Engineering_Name LIKE '%Rotem%' Then 'ROTEM_SP'  END AS product 
from ld_New_Era_df_view ) a 
where product is not null
'''
ld_dim_blankets_name = spark.sql(sql)

# COMMAND ----------

ld_fact_New_Era_parameters_df = ld_fact_New_Era_all_blankets_df.join(ld_dim_blankets_name,ld_dim_blankets_name.Product_Engineering_Name == ld_fact_New_Era_all_blankets_df.Product_Engineering_Name,"inner").drop(ld_fact_New_Era_all_blankets_df.Product_Engineering_Name)

# COMMAND ----------

ld_fact_New_Era_parameters_df.write.mode("overwrite").saveAsTable("blankets_db.ld_new_era_all_blankets")
...
SELECT
  ...
  BLK_Quality_Status_Flag,
  PR.*
  FROM   blankets_db.ld_new_era_all_blankets  as NE
  inner join blankets_db.ld_fact_parameters_run_level_pivot PR
  on NE.Run_Number = PR.Batch;
'''
ce_fact_new_era_parameters = spark.sql(sql)
...
#Blankets_Production_lifespan_df.write.mode("overwrite").parquet(certified_pit + "/Blankets_Production_lifespan/ce_fact_Blankets_Production_lifespan_pandas.parquet")
ce_fact_new_era_parameters.coalesce(1).write.format("parquet") \
.option("path",certified_pit + "/Blankets_Production_new_era/ce_fact_new_era_parameters/") \
.mode("overwrite") \
.saveAsTable("blankets_db.ce_fact_new_era_parameters")